"""
Same joint (base + global covariate) test as joint_model.py, but using a
SAFE, BOUNDED global feature instead of raw mean EV: Fd = fraction of the
scene's unclipped pixels below -3 EV ("deep shadow fraction" / a dynamic-
range proxy). Raw mean/median EV blows up towards -infinity for any scene
with black-crushed pixels (log domain near zero), which is not a usable
render-time covariate -- Fd is bounded in [0,1] by construction, so
gamma*Fd can never destabilise the monotonicity of the recovery mapping the
way an unbounded EV statistic could.
"""
import os, glob
import numpy as np
import fit
...
def load_group(prefix):
    ...
    for stem in stems:
        ...
        vis = d["ev_n"][~d["clip_n"]]
        d["M"] = float(np.mean(vis < -3.0))   # Fd: deep-shadow fraction, in [0,1]
        scenes.append(d)
    return stems, scenes
...
def fit_joint(scenes, bases, M0):
    ...
    for d in (-1, 1):
        ...
        n_sb = np.array(n_sb); sy_sb = np.array(sy_sb); M_arr = np.array(M_list)
...
def rms_two(scenes, bases, curves_old, curves_new, M0):
    ...
    print(f"    old(base-only)={np.sqrt(sse_old/N):.3f}  new(base+Fd)={np.sqrt(sse_new/N):.3f}  nothing={np.sqrt(sse_base/N):.3f}")
    for s in SLIDERS:
...
def main():
    ...
    print("Fd (deep-shadow fraction) range D1:", min(sc['M'] for sc in d1), max(sc['M'] for sc in d1))
    print("Fd range D2:", min(sc['M'] for sc in d2), max(sc['M'] for sc in d2))
    R = 8
    ...
    print(f"M0 (D1 mean Fd, reference point) = {M0:.3f}")
    ...
    for d in (-1, 1):
        print(f"  direction {d:+d}: gamma = {joint[d][1]:+.4f} EV per unit of Fd (0..1)")
